...
// D1Database type (Cloudflare Workers)
...
export async function verifyToken(token: string, jwtSecret: string): Promise<JWTPayload> {
  ...
  return {
    ...
    role: payload.role as 'superadmin' | 'agency' | 'admin' | 'viewer',
 ...
 */
export function canModify(user: JWTPayload | AdminUser): boolean {
...
export function canManageUsers(role: string | undefined): boolean {
  return role === 'superadmin' || role === 'agency' || role === 'admin';
}

/**
 * Create session in database
 */
export async function createSession(db: D1Database, userId: string, sessionId: string, expiresAt: Date): Promise<void> {
  await db
 ...
 * Get user by ID
 */
export async function getUserById(db: D1Database, id: string): Promise<AdminUser | null> {
  const result = await db.prepare(`SELECT * FROM admin_users WHERE id = ?`).bind(id).first<AdminUser>();
